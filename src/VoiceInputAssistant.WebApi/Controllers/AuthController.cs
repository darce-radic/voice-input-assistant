    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateProfile([FromBody] UpdateProfileRequest request)
        {
            try
            {
                ...
                user.Organization = request.Organization ?? string.Empty;
        ...
        public async Task<IActionResult> ResetPasswordConfirm([FromBody] ResetPasswordConfirmRequest request)
        {
            try
            {
                ...
                var isValidToken = await _tokenService.ValidatePasswordResetTokenAsync(request.Token, user.Id);
                if (!isValidToken)
                {
        ...
        public async Task<IActionResult> ConfirmEmail([FromBody] ConfirmEmailRequest request)
        {
            try
            {
                ...
                var isValidToken = await _tokenService.ValidateEmailConfirmationTokenAsync(request.Token, request.UserId);
                if (!isValidToken)
                {
                    return BadRequest(new { error = "Invalid or expired confirmation token" });
                }

                await _userService.ConfirmEmailAsync(request.UserId);
